...
`.level-nav-btn`'s base size (~18×16px) is fine for a mouse but too small a touch target on its own — a `(hover: none) and (pointer: coarse)` media query bumps both buttons up to 42×42px (and `#level-reset` below to 46×46px) specifically for touch, since the whole `#ui-canvas` overlay these buttons live in is already CSS-scaled to match the canvas (see DOM Overlay Pattern), so a fixed logical size renders at a fairly consistent physical size across devices regardless of viewport.
