...
`buildLevelTextures()` generates 4 `pond_corner_*` variants (`tl`/`tr`/`bl`/`br`) alongside the plain `pond` tile — the same water pattern (factored into a shared `drawPondBase()` closure) with a grass-colored (`C.bg`) triangular notch cut into the outer corner. `buildObstacleLayer()` stamps these at the 4 corners of a pond's bounding box instead of the plain tile (checking `dr`/`dc` against `cH - 1`/`cw - 1` within the same per-cell stamping loop bushes already use), so the overall shape reads as rounded rather than a sharp rectangle — collision (`obstacleGrid`) stays the full rectangle regardless; this is purely visual. A separate pass after the main obstacle loop scatters `cattail` texture (~22% chance per cell) across the plain-grass ring bordering the pond (one cell out from its bounding box on all sides, skipping the pond itself and any non-grass cells) — a shoreline accent, purely decorative like the lily pads, with no effect on collision or mowability.
