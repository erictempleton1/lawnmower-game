...
- `TREE_TYPES` (`tree_round`, `tree_evergreen`, `tree_willow`) — one is picked at random per contiguous tree cluster in `buildObstacleLayer()` (not per 2×2 sub-block), so a single clump of trees reads as one coherent species. All three keep the trunk in the same local y=18..31 footprint so the shared `ty + 8` trunk-collision offset works for any of them without a per-type adjustment.
