...
```
CELL = 16px
YARD_X = 5, YARD_Y = 3 (border offset)
YARD_COLS = 14 (fixed — matches every level's authored width)
BASE_YARD_ROWS = 12 (authored level height)
```
`YARD_ROWS` (and so `ROWS`/`H`) is **computed at load** by `computeYardRows()` — on portrait + touch devices it grows past 12 to fill available vertical space (screen height minus `CONTROL_RESERVE_PX`, ~1 inch, reserved for the D-pad), clamped to `MAX_YARD_ROWS`. Desktop and mobile landscape always get the fixed 12 rows. This is a one-time computation at page load; it does not live-recompute on resize or orientation change. `normalizeMap()` pads each authored level's map out to `YARD_ROWS` with plain grass, centering the original trees/gardens vertically — level files themselves stay a fixed 14×12 and never need to change. Only rows grow (never columns), so the mower stays a fixed single-cell size — there's no swath-widening or world-scaling here, by design (see git history around 2026-07-18 for why that combination was reverted).

`YARD_X` (5) and `YARD_COLS` (14) were chosen together so `COLS` (`= YARD_COLS + YARD_X*2` = 24) — and so the overall canvas width `W` — stays exactly what it was before the yard was narrowed (`YARD_X` 3, `YARD_COLS` 18, same `COLS` of 24). Narrowing the mowable area is purely a matter of shifting where the yard/border boundary falls within an unchanged canvas, not shrinking the game window. `normalizeMap()`'s pad logic (`padLeft = Math.floor((YARD_COLS - baseCols) / 2)`) already generalizes to *cropping* when the target is narrower than the authored map (a negative `padLeft`), which is what made the original 18-wide levels able to shrink to 14 at all — but the three level files were re-authored to exactly 14 columns instead of relying on that runtime crop, so `mowedTextureKey()`'s ring/stripe centering and every level's hand-placed obstacle spacing (still ≥2-cell gaps between clusters, re-verified after the resize) stay exactly what's on screen.
...
The left/right sides get a **second, inner** tree/pine row (`innerTreeX`/`innerTreeXR`, offset in from `yardL`/`yardR` rather than the canvas edge) alongside the original outer row (fixed 20px from the canvas edge, `margin`). The outer row alone was deep enough to reach the yard boundary back when the border was narrower (`YARD_X` 3) — a single ~44px-wide tree centered 20px from the edge already spans almost the full old 48px margin. Widening the border (`YARD_X` 5, 80px margin) without a second row would leave a bare gap of just grass/wildflowers between that outer row and the yard; the inner row (skipped entirely if the border's too narrow to fit one without the two colliding, via an `innerTreeX > margin + 30` guard) fills that gap so the wider margin still reads as an actual forest depth, not a thin tree-line with empty space behind it.
