...
## Mowed Texture Shapes
`buildMowedTextures()` generates three variants per deck height: `mowed_H_v` (12px wide × full-CELL tall), `mowed_H_h` (full-CELL wide × 12px tall), and `mowed_H_full` (16×16). The 12px dimension matches the mower's actual visual width (its wheel-to-wheel span in `drawPlayer()`) so a mowed patch never reads as wider than whatever mowed it — but narrowing *both* dimensions left a visible gap of unmowed grass between consecutive cells in a straight pass, so only the dimension perpendicular to travel narrows; the along-travel dimension stays full-CELL and tiles seamlessly. `mowAt()` picks the shape via `getMowShapeCode()` (`MOW_SHAPE_KEYS` = `['v','h','full']`), based on `this.player.moveDX/moveDY` (the actual last movement vector, tracked separately from `player.dir` since `dir` only ever picks one of 4 facings). True diagonal movement (both axes above a small threshold) doesn't align with either strip — diagonally-stepped cells only touch at a corner, not an edge — so it falls back to `full` there; mostly-straight movement picks `v`/`h` from whichever axis dominates. The chosen code is recorded in `this.cellShape[r][c]` so `rebuildMowedRT()` (used after a sprinkler revert) can re-stamp each cell with the shape it was originally mowed with. `full` is also always used by `checkClusterCompletion()`'s garden auto-mow — those cells are permanently hidden under the garden's own obstacle-layer texture anyway (depth 3, above the mowed layer's depth 1), so no narrowing makes sense there.
